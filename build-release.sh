...
# Copiar README y VERSION si existen
if [ -f "README_INSTALACION.md" ]; then
    echo "  Copiando README_INSTALACION.md..."
    cp README_INSTALACION.md "$RELEASE_DIR/"
fi

if [ -f "VERSION" ]; then
    echo "  Copiando VERSION..."
...
if [ -d "$RELEASE_DIR/app/config" ]; then
    ...
    # Eliminar solo config.php generado (NO paths.php ni config.example.php)
